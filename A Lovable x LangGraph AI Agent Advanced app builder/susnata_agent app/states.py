#states for my LangGraph
# Defining  schema for files imported from pydantic
...
# Define schema for overall plan
class Plan(BaseModel):
    name: str = Field(
        description="The name of the app to be built"
    )
    description: str = Field(
        description="A one-line description of the app to be built, e.g. 'A web application for managing tasks'"
    )
    techstack: str = Field(
        description="The tech stack to be used for the app, e.g. 'python', 'javascript', 'react', 'flask', etc."
    )
    features: list[str] = Field(
        description="A list of features that the app should have, e.g. 'user authentication', 'dark mode', 'dashboard'"
    )
    files: list[File] = Field(
        description="A list of files to be created, each with a 'path' and 'purpose'"
    )

# Generate structured plan from the LLM
# resp = llm.with_structured_output(Plan).invoke(prompt)
#
# print(resp)

#Plan >file like css for style look and feel, java script for implementation,html
#And it purpose frontend backend
...
class TaskPlan(BaseModel):
    ...
    model_config = ConfigDict(extra="allow")   #Let me  add arbitary attributes add extra elements object of class


class CoderState(BaseModel):
    task_plan: TaskPlan = Field(description="The plan for the task to be implemented")
    current_step_idx: int = Field(0, description="The index of the current step in the implementation steps")
    current_file_content: str = Field(None, description="The content of the file currently being edited or created")